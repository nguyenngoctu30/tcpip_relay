...
byte mac[] = {0x54, 0x32, 0x04, 0x0C, 0xE6, 0x02};

IPAddress default_ip(192,168,1,212);
...
// Biến trạng thái relay/device
int relay_state = 0;
int last_relay_state = -1;  // Để detect thay đổi
...
void updateRelayState(){
  // Chân 20 INPUT_PULLUP:
  // - Nếu không nối GND (HIGH) → relay_state = 0
  // - Nếu nối GND (LOW) → relay_state = 1
  int pin_state = digitalRead(RELAY_PIN);
  relay_state = !pin_state;  // Đảo ngược: HIGH=0, LOW=1
  
  // Nếu trạng thái thay đổi, publish ngay
...
bool testMQTT(){
  ...
  // QUAN TRỌNG: Gọi loop() nhiều lần để hoàn tất MQTT CONNACK handshake
  // Sau connect() chỉ là TCP connection, MQTT protocol cần loop() để hoàn tất
  for(int i=0; i<15; i++){
    client.loop();
    delay(100);
  }
...
void applyNewIP(IPAddress ip, IPAddress dns, IPAddress gw, IPAddress sn){
  ...
  if(testMQTT()){
    ...
    Serial.println("IP FAIL → ROLLBACK");
    applyNetwork(backup_ip,backup_dns,backup_gateway,backup_subnet);
  }
}
...
void callback(char* topic, byte* payload, unsigned int len){
  ...
  if(String(topic)==cmd_topic){
    ...
    if(cmd == 0 || cmd == 1){
      relay_state = cmd;
      Serial.print("Relay state updated: ");
      Serial.println(relay_state);
      
      // Publish trạng thái mới lên data_topic
      client.publish(data_topic.c_str(), String(relay_state).c_str(), true);
    }
    return;
  }
...
void reconnect(){
  ...
  while(!client.connected()){
    ...
    if(client.connect(clientId.c_str(),status_topic.c_str(),1,true,"OFFLINE")){
      ...
      // QUAN TRỌNG: Gọi loop() nhiều lần để hoàn tất MQTT CONNACK
      for(int i=0; i<15; i++){
        client.loop();
        delay(100);
      }
      ...
      Serial.println(ip_topic);
      client.publish(ip_topic.c_str(),getCurrentIPString().c_str(),true);
      delay(200);
      // Publish trạng thái relay/device
      Serial.print("Publishing relay state to ");
...
void setup(){
  ...
  client.setSocketTimeout(10);  // Tăng timeout từ 5 lên 10 giây
  client.setCallback(callback);

  Serial.println("\n=== ESP32 + W5500 READY ===");
  Serial.print("IP: ");
  Serial.println(getCurrentIPString());
}
